...
let tick = 0;
function blank() {
  return { view: "home", name: "", code: "", tab: "dossier", killOpen: false, accuseOpen: false, accusedId: null, explain: "", guest: "", party: null, seatId: null, toast: "", hold: false };
}
function load() {
  try { return JSON.parse(localStorage.getItem(KEY) || "null"); } catch { return null; }
}
function save() {
  const copy = { ...S, toast: "", hold: false };
...
function me() { return S.party?.players.find((p) => p.id === S.seatId) || null; }
function byId(id) { return S.party?.players.find((p) => p.id === id) || null; }
function targetOf(p) { return p?.targetId ? byId(p.targetId) : null; }
function pendingOf(p) { return S.party?.claims.find((c) => c.killerId === p.id && c.status === "pending") || null; }
function pendingAll() { return (S.party?.claims || []).filter((c) => c.status === "pending"); }
function startChain(party) {
  const order = shuffle(party.players);
  const missions = pickMissions(order.length);
  order.forEach((p, i) => {
    const t = order[(i + 1) % order.length];
    p.targetId = t.id; p.mission = missions[i]; p.isAlive = true; p.discovered = false; p.killCount = 0;
  });
  party.status = "playing"; party.winnerId = null; party.startedAt = now();
}
function createParty(name, demo) {
  const host = player(name, true);
  const party = { id: uid(), code: makeCode(), status: "lobby", isDemo: !!demo, hostId: host.id, players: [host], claims: [], winnerId: null, winnerName: null };
  if (demo) DEMO.slice(1).forEach((n) => party.players.push(player(n, false)));
  S.party = party; S.seatId = host.id; S.name = name;
...
function declareKill(explanation) {
  const p = me();
  if (!p?.isAlive) throw new Error("Tu as déjà été éliminé.");
  ...
  S.party.claims.push({ id: uid(), killerId: p.id, victimId: p.targetId, explanation: explanation.trim(), status: "pending", createdAt: now(), mission: p.mission });
}
function resolveKill(claimId, approve) {
  ...
  const killer = byId(c.killerId); const victim = byId(c.victimId);
  if (!killer || !victim || !killer.isAlive || !victim.isAlive || killer.targetId !== victim.id) {
    c.status = "refused"; throw new Error("Le kill n'est plus valable.");
  }
  const inheritedTarget = victim.targetId; const inheritedMission = victim.mission;
  victim.isAlive = false; victim.targetId = null; victim.mission = null;
  killer.targetId = inheritedTarget; killer.mission = inheritedMission; killer.killCount += 1;
  c.status = "validated";
  const alive = S.party.players.filter((x) => x.isAlive);
  if (alive.length <= 1 || inheritedTarget === killer.id) {
    S.party.status = "finished"; S.party.winnerId = killer.id; S.party.winnerName = killer.name; S.view = "finale";
  }
}
...
function dagger() {
  return '<svg class="mark" viewBox="0 0 32 32" aria-hidden="true"><polygon points="16,2 23,14 16,30 9,14" fill="currentColor"/><polygon points="16,8 19.5,14 16,22 12.5,14" fill="#c42b3a"/></svg>';
}
